...
export default function CategoryProductDetail({ product }) {
  console.log(product);
  return (
...
export async function getStaticPaths() {
  const response = await axios.get(
    "http://localhost:8000/api/products/category/list"
  );
...
export async function getStaticProps({ params }) {
  const { id } = params;
  const response = await axios.get(
    `http://localhost:8000/api/products/category/detail/${id}`
  );
