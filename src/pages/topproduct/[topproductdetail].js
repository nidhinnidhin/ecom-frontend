...
export default function TopProductDetail({ product }) {
  console.log(product)
  return (
...
export async function getStaticPaths() {
  const res = await axios.get("http://localhost:8000/topproduct/topproductlist/");
  const products = res.data;

  const paths = products.map((product) => ({
    params: { topproductdetail: product.id.toString() },
  }));
...
export async function getStaticProps({ params }) {
  const { topproductdetail } = params;

  const res = await axios.get(`http://localhost:8000/topproduct/topproductdetail/${topproductdetail}/`);
